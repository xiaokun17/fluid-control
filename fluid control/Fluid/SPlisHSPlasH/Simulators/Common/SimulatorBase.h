...
#include "SPlisHSPlasH/ControlBoundaryModel.h"
...
namespace SPH
{
	...
	protected:
		...
		bool m_enablemyPartioCSVExport;
		bool m_enablePicExport;
	...
	public:
		...
		static int ENUM_WALLS_GEOMETRY_RIG;
		static int ENUM_WALLS_CONTROL_PARTICLES_NO_WALLS;
		static int ENUM_WALLS_Target_View;
		...
		void myposdataExport();//20200907-zj-posfile write
		void picExport();

		void writeParticles(const std::string &fileName, FluidModel *model);
		void mywriteParticles(const std::string& fileName, FluidModel* model);//20200907-zj-posfile write
		void controlInfo(const std::string& fileName, ControlBoundaryModel* model);
		void step();
		void reset();
