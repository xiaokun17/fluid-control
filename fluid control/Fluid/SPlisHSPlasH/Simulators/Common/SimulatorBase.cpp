...
int SimulatorBase::ENUM_WALLS_GEOMETRY_RIG = -1;

int SimulatorBase::ENUM_WALLS_CONTROL_PARTICLES_NO_WALLS = -1;
int SimulatorBase::ENUM_WALLS_Target_View = -1;
...
SimulatorBase::SimulatorBase()
{
	...
	m_enablemyPartioCSVExport = false;//2020-09-07-zj-writefile
	m_enablePicExport = false;
...
void SimulatorBase::initParameters()
{
	...
	enumParam->addEnumValue("Display Rig(no walls)", ENUM_WALLS_GEOMETRY_RIG);
	enumParam->addEnumValue("Particles&Control(nowall)", ENUM_WALLS_CONTROL_PARTICLES_NO_WALLS);//2020-10 zj Ô¤ÀÀ¿ØÖÆÁ£×ÓµÄäÖÈ¾
	enumParam->addEnumValue("Target View(no wall)", ENUM_WALLS_Target_View);

	PARTIO_EXPORT = createBoolParameter("PicExport", "Pic Export", &m_enablePicExport);
	setGroup(PARTIO_EXPORT, "Export");
	setDescription(PARTIO_EXPORT, "Enable/disable pic export");

	PARTIO_EXPORT = createBoolParameter("CSVExport", "CSV export", &m_enablemyPartioCSVExport);//2020-09-07-zj-writefile
	setGroup(PARTIO_EXPORT, "Export");
	setDescription(PARTIO_EXPORT, "Enable/disable CSV export.");
...
void SimulatorBase::init(int argc, char **argv, const char *simName)
{
	...
	m_sceneFile = getDataPath() + "/Scenes/Controlf.json";
	...
	if (sim2D)
		...
		MiniGL::setViewport(40.0, 0.1f, 500.0, Vector3r(0.0, 0.7, 10.0), Vector3r(0.0, 0.0, 0.0));
...
void SimulatorBase::initFluidData()
{
	...
	for (auto it = fluidIDs.begin(); it != fluidIDs.end(); it++)
	{
		...
		unsigned int maxEmitterParticles = 100000;
...
void SimulatorBase::myposdataExport()
{
	std::string exportPath = FileSystem::normalizePath(m_outputPath + "/pos");
	FileSystem::makeDirs(exportPath);
	...
	for (unsigned int i = 0; i < sim->numberOfFluidModels(); i++)
	{
		FluidModel* model = sim->getFluidModel(i);
		std::string fileName = "ParticleData";
		fileName = fileName + "_" + model->getId() + "_" + std::to_string(m_frameCounter) + ".csv";
		std::string exportFileName = FileSystem::normalizePath(exportPath + "/" + fileName);
		mywriteParticles(exportFileName, model);
		//writeParticles(exportFileName, model);
	}
	//for (unsigned int i = 0; i < sim->numberOfControlBoundaryModels(); i++)
	//{
	//	
	//	ControlBoundaryModel* model = sim->getControlBoundaryModel(i);
	//	std::string fileName = "Control";
	//	fileName = fileName + "_" + std::to_string(m_frameCounter) + ".csv";
	//	std::string exportFileName = FileSystem::normalizePath(exportPath + "/" + fileName);
	//	controlInfo(exportFileName, model);
	//}

}

void SimulatorBase::picExport()
{
...
void SimulatorBase::controlInfo(const std::string& fileName, ControlBoundaryModel* model)
{
	...
	for (unsigned int i = 0; i < numParticles; i++)
	{
		...
		fprintf(f, "%f,%f,%f,%f\n", (float)x[0], (float)x[1], (float)x[2],(float)Ci);
	}
	fclose(f);
}

void SimulatorBase::mywriteParticles(const std::string& fileName, FluidModel* model)
{
	const unsigned int numParticles = model->numActiveParticles();
	FILE* f;
	f = fopen(fileName.c_str(), "wb");
...
void SimulatorBase::step()
{
	...
	else if (m_enablemyPartioCSVExport&&!m_enablePicExport)
	{
		if (TimeManager::getCurrent()->getTime() >= m_nextFrameTime)
		{
			m_nextFrameTime += static_cast<Real>(1.0) / m_framesPerSecond;
	...
	else if (m_enablePicExport&&!m_enablemyPartioCSVExport)
	{
		if (TimeManager::getCurrent()->getTime() >= m_nextFrameTime)
		{
			m_nextFrameTime += static_cast<Real>(1.0) / m_framesPerSecond;
