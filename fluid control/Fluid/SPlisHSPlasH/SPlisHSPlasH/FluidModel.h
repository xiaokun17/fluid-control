...
{	
	...
	enum class VorticityMethods { None = 0, Micropolar, VorticityConfinement, VorticityRefinementm, NumVorticityMethods};
	...
	*/
	class FluidModel : public GenParam::ParameterObject
	{
		...
		protected:
			...
			std::vector<int> ri_index;//control spring:reference ri
			std::vector<int> timelife;//control spring:life time
			Real m_V;
		...
		public:
			...
			unsigned int getCponitSetIndex() const { return c_pointSetIndex; }
			...
			FORCE_INLINE const int& getri(const unsigned int i) const
			{
				return ri_index[i];
			}

			FORCE_INLINE int& getri(const unsigned int i)
			{
				return ri_index[i];
			}

			FORCE_INLINE void setri(const unsigned int i, const int& val)
			{
				ri_index[i] = val;
			}

			FORCE_INLINE const int& getTimelife(const unsigned int i) const
			{
				return timelife[i];
			}

			FORCE_INLINE int& getTimelife(const unsigned int i)
			{
				return timelife[i];
			}

			FORCE_INLINE void setTimelife(const unsigned int i, const int& val)
			{
				timelife[i] = val;
			}
