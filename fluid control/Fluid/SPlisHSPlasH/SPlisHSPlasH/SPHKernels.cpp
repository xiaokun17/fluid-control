#include "SPHKernels.h"

using namespace SPH;

Real CubicKernel::m_radius;
Real CubicKernel::m_k;
Real CubicKernel::m_l;
Real CubicKernel::m_W_zero;
Real CubicKernel::c_radius;
Real CubicKernel::c_k;
Real CubicKernel::c_l;
Real CubicKernel::c_W_zero;

Real Poly6Kernel::m_radius;
Real Poly6Kernel::m_k;
Real Poly6Kernel::m_l;
Real Poly6Kernel::m_m;
Real Poly6Kernel::m_W_zero;
Real Poly6Kernel::c_radius;
Real Poly6Kernel::c_k;
Real Poly6Kernel::c_l;
Real Poly6Kernel::c_m;
Real Poly6Kernel::c_W_zero;

Real SpikyKernel::m_radius;
Real SpikyKernel::m_k;
Real SpikyKernel::m_l;
Real SpikyKernel::m_W_zero;
Real SpikyKernel::c_radius;
Real SpikyKernel::c_k;
Real SpikyKernel::c_l;
Real SpikyKernel::c_W_zero;

Real WendlandQuinticC2Kernel::m_radius;
Real WendlandQuinticC2Kernel::m_k;
Real WendlandQuinticC2Kernel::m_l;
Real WendlandQuinticC2Kernel::m_W_zero;
Real WendlandQuinticC2Kernel::c_radius;
Real WendlandQuinticC2Kernel::c_k;
Real WendlandQuinticC2Kernel::c_l;
Real WendlandQuinticC2Kernel::c_W_zero;

Real CohesionKernel::m_radius;
Real CohesionKernel::m_k;
Real CohesionKernel::m_c;
Real CohesionKernel::m_W_zero;
Real CohesionKernel::c_radius;
Real CohesionKernel::c_k;
Real CohesionKernel::c_c;
Real CohesionKernel::c_W_zero;

Real AdhesionKernel::m_radius;
Real AdhesionKernel::m_k;
Real AdhesionKernel::m_W_zero;
Real AdhesionKernel::c_radius;
Real AdhesionKernel::c_k;
Real AdhesionKernel::c_W_zero;

Real CubicKernel2D::m_radius;
Real CubicKernel2D::m_k;
Real CubicKernel2D::m_l;
Real CubicKernel2D::m_W_zero;
Real CubicKernel2D::c_radius;
Real CubicKernel2D::c_k;
Real CubicKernel2D::c_l;
Real CubicKernel2D::c_W_zero;

Real WendlandQuinticC2Kernel2D::m_radius;
Real WendlandQuinticC2Kernel2D::m_k;
Real WendlandQuinticC2Kernel2D::m_l;
Real WendlandQuinticC2Kernel2D::m_W_zero;
Real WendlandQuinticC2Kernel2D::c_radius;
Real WendlandQuinticC2Kernel2D::c_k;
Real WendlandQuinticC2Kernel2D::c_l;
Real WendlandQuinticC2Kernel2D::c_W_zero;

