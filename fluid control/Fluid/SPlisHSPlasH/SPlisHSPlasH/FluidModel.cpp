...
#include "Vorticity/VorticityRefinement_Liu2020.h" //2021-1
...
int FluidModel::ENUM_VORTICITY_VR = -1;
...
void FluidModel::initParameters()
{
	...
	enumParam->addEnumValue("Vorticity refinement", ENUM_VORTICITY_VR);
...
void FluidModel::resizeFluidParticles(const unsigned int newSize)
{
	...
	ri_index.resize(newSize);
	timelife.resize(newSize);
}
...
void FluidModel::initModel(const std::string &id, const unsigned int nFluidParticles, Vector3r* fluidParticles, Vector3r* fluidVelocities, const unsigned int nMaxEmitterParticles)
{
	...
	{
		NeighborhoodSearch* cneighborhoodSearch = Simulation::getCurrent()->getCNeighborhoodSearch();
		c_pointSetIndex = cneighborhoodSearch->add_point_set(&getPosition(0)[0], nFluidParticles, true, true, true, this);
	}
...
void FluidModel::performNeighborhoodSearchSort()
{
	...
	auto const& d = neighborhoodSearch->point_set(c_pointSetIndex);
	d.sort_field(&m_x[0]);
	d.sort_field(&m_v[0]);
	d.sort_field(&m_a[0]);
	d.sort_field(&m_masses[0]);
	d.sort_field(&m_density[0]);
	d.sort_field(&timelife[0]);

	NeighborhoodSearch* cneighborhoodSearch = Simulation::getCurrent()->getCNeighborhoodSearch();
	auto const& cd = cneighborhoodSearch->point_set(c_pointSetIndex);
	cd.sort_field(&m_x[0]);
	cd.sort_field(&m_v[0]);
	cd.sort_field(&m_a[0]);
	cd.sort_field(&m_masses[0]);
	cd.sort_field(&m_density[0]);
	cd.sort_field(&timelife[0]);
...
{
	if (m_vorticity)//&& m_vorticityMethod != VorticityMethods::VorticityRefinementm)
		m_vorticity->step();
}
...
void FluidModel::setVorticityMethod(const int val)
{
	...
	else if (m_vorticityMethod == VorticityMethods::VorticityRefinementm)
		m_vorticity = new VorticityRefinement_Liu2020(this);
